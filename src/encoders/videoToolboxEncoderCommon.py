...
def make_video_toolbox_encoder(codec_name, extension, encoder_label, extra_encoder_flags=None):
  extra_encoder_flags = list(extra_encoder_flags or [])
  ...
  def encoder(inputsList, outputPathName, filenamePrefix, filtercommand, options, totalEncodedSeconds, totalExpectedEncodedSeconds, statusCallback, requestId=None, encodeStageFilter='null', globalOptions={}, packageglobalStatusCallback=print, startEndTimestamps=None):
    ...
    videoFileName, logFilePath, filterFilePath, tempVideoFilePath, videoFilePath = getFreeNameForFileAndLog(filenamePrefix, extension, requestId)
    ...
    def encoderFunction(br, passNumber, passReason, passPhase=0, requestId=None, widthReduction=0.0, bufsize=None, cqMode=False):
      ...
      if widthReduction > 0.0:
        encodefiltercommand = filtercommand + ',{encodeStageFilter},[outv]scale=iw*(1-{widthReduction}):ih*(1-{widthReduction}):flags=bicubic[outvfinal]'.format(
          encodeStageFilter=encodeStageFilter,
      ...
      if startEndTimestamps is None:
        ffmpegcommand += ['-shortest', '-copyts', '-start_at_zero']

      ffmpegcommand += extra_encoder_flags
      ffmpegcommand += ['-c:v', codec_name, '-pix_fmt', 'yuv420p', '-b:v', str(br), '-movflags', '+faststart']
      ...
      if options.get('audioChannels') == 'No audio' or passPhase == 1:
        ...
        ffmpegcommand += ['-ac', '2', '-ar', str(44100), '-b:a', str(audoBitrate)]
      elif 'Mono' in options.get('audioChannels', ''):
        ffmpegcommand += ['-ac', '1', '-ar', str(44100), '-b:a', str(audoBitrate)]
      elif 'Copy' in options.get('audioChannels', ''):
      ...
      psnr, returnCode = logffmpegEncodeProgress(
        proc,
        'Pass {} {} {}'.format(passNumber, passReason, videoFileName),
        totalEncodedSeconds,
        totalExpectedEncodedSeconds,
        encoderStatusCallback,
        passNumber=passPhase,
        requestId=requestId,
        tempVideoPath=tempVideoFilePath,
        options=options,
      )
    ...
    finalFilenameConfirmed = optimiser(
